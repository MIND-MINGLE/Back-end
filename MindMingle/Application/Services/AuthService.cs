...
namespace Application.Services
{
    public class AuthService : IAuthService
    {
        ...
        {
            ApiResponse response = new ApiResponse();
            var user = await _unitOfWorks.AccountRepo.GetAsync(x => x.Email == request.Email);
            if (user != null)
            {
                if (user.IsDisabled)
                {
            ...
            {
                var existedUser = await _unitOfWorks.AccountRepo.GetAsync(x => x.Email == request.Email);
                if (existedUser != null)
                {
                ...
                else
                {
                    Account newAccount = new Account()
                    {
                        ...
                        AccountId = request.Sub,
                        Email = request.Email,
                        AccountName = request.Family_name + "_" + request.Given_name,
                        Password = request.Sub,
                        IsDisabled = request.RoleId.Equals("doc") || request.RoleId.Equals("agent"),
                        //Disable business account on default, admin verified
                        Avatar = request.Picture,
                        IsEmailVerified = request.Email_verified
                    };
                    ...
                    switch (request.RoleId)
                    {
                        case "seeker": {
                                ...
                                await _unitOfWorks.PatientRepo.AddAsync(newPatient);
                                await _unitOfWorks.SaveChangeAsync();
                                break;
                            }
                        case "doc": {
                                ...
                                await _unitOfWorks.TherapistRepo.AddAsync(newTherapist);
                                await _unitOfWorks.SaveChangeAsync();
                                break; }
                        case "agent": {
                                //TODO Add Agent
                        ...
                        default:
                            {
                                response.SetBadRequest(message: "Role Not Found!");
                                return response;
                            }
                    }
                    var token = _tokenService.CreateToken(newAccount);
                    return response.SetOk(token);
